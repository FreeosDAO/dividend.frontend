...
export default {
  ...
  methods: {
    ...
    conditions () {
      // todo rewrite (proposal not voted here) ||
      ...
      this.meVoted = false
      console.log('this.alreadyVotedName', this.alreadyVotedName, 'this.accountName', this.accountName,
    ...
    isProposalActive () { // Is proposal active ? - it means not expired ?
      // TODO Rewrite / use as timer
      if ((this.eosaccount !== 'empty') && (this.account !== 'erased')) { // todo required ??
